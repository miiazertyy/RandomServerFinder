 ...
 *
 * <p>The signature has moved twice. 1.20.1 takes {@code (ServerInfo, Runnable)}, later versions
 * added a second callback for the latency reply, and 1.21.11 added a {@code NetworkingBackend}
 * argument on top. Referring to any one shape directly would pin the mod to one version, so the
 * best available overload is looked up once by reflection and reused.
 *
 * <p>Callers must not assume the latency callback runs: on 1.20.1 there is nowhere to pass it.
 *
...
public final class PingerCompat {
	 ...
	 * @param onPong   run when the latency reply arrives, if this version reports one at all
	 * @throws Exception whatever the underlying call throws, including {@code UnknownHostException}
	 */
	public static void add(MultiplayerServerListPinger pinger, ServerInfo info, Runnable saver, Runnable onPong)
			throws Exception {
		resolve();
		if (addMethod == null) {
			throw new IllegalStateException(
					"No usable MultiplayerServerListPinger.add on this version; candidates were: " + signatures);
		}

		try {
			switch (addMethod.getParameterCount()) {
				case 4 -> addMethod.invoke(pinger, info, saver, onPong, networkingBackend);
				case 3 -> addMethod.invoke(pinger, info, saver, onPong);
				// 1.20.1: no latency callback exists, so onPong is dropped rather than faked. The
				// feed reads completion off the server info instead.
				default -> addMethod.invoke(pinger, info, saver);
			}
	...
	private static java.util.List<Method> candidates() {
		...
		java.util.Collections.addAll(all, MultiplayerServerListPinger.class.getMethods());
		for (Method declared : MultiplayerServerListPinger.class.getDeclaredMethods()) {
	...
	private static synchronized void resolve() {
		...
		for (Method method : candidates()) {
			...
			if (parameters.length == 4) {
				...
				if (threeArg == null) threeArg = method;
			} else if (twoArg == null) {
				twoArg = method;
			}
		}

		// Only settle for an older signature once every richer candidate has been ruled out — an
		// earlier version of this loop let a failed candidate clear a good one it had already found.
		Method fallback = threeArg != null ? threeArg : twoArg;
		if (fallback != null) {
			addMethod = fallback;
	...
	/**
	 * {@code (ServerInfo, Runnable)}, optionally a second {@code Runnable}, optionally the backend.
	 *
	 * <p>Two arguments is 1.20.1, three is 1.20.2 through 1.21.10, four is 1.21.11 and later.
	 */
	private static boolean looksLikePing(Class<?>[] parameters) {
		if (parameters.length < 2 || parameters.length > 4) return false;
		if (!parameters[0].isAssignableFrom(ServerInfo.class) || parameters[1] != Runnable.class) {
			return false;
		}
		return parameters.length == 2 || parameters[2] == Runnable.class;
	}
	...
	private static Object createBackend(Class<?> backendType) {
		...
		if (client != null && client.options != null) {
			nativeTransport = client.options.shouldUseNativeTransport();
		}
