...
public class LocalServerFeed {
	...
	private static final class Candidate {
		...
		/** Only ever read or written on the client thread. */
		PingState state = PingState.PINGING;
	...
	private void startPing(long ip, int port, boolean bedrock, boolean neighbour) {
		...
		executor.submit(() -> {
			try {
				...
				client.execute(() -> candidate.state = PingState.UNREACHABLE);
			}
		});
	}
	...
	private void harvest() {
		...
		while (it.hasNext()) {
			Candidate c = it.next();
			PingState status = c.state;
			boolean timedOut = now - c.startedAt > PING_TIMEOUT_MS;

			// A server that sent its details but no latency reply is still perfectly joinable, and on
			// 1.20.1 that is every one of them: the pinger has nowhere to hang a latency callback, so
			// the reply is recognised by what it left behind instead. Without this every candidate
			// would sit in flight until it timed out, answered or not, and the scan would crawl.
			boolean responded = c.answered || c.info.players != null || c.info.getFavicon() != null
					|| c.info.ping > 0L;

			if (status == PingState.PINGING && !responded && !timedOut) {
				continue;
			}
			it.remove();

			if (!responded || status == PingState.UNREACHABLE) {
				continue;
			}
			...
			c.state = ViaFabricPlusBridge.isAvailable()
					|| c.info.protocolVersion == net.minecraft.SharedConstants.getGameVersion().getProtocolVersion()
					? PingState.SUCCESSFUL
					: PingState.INCOMPATIBLE;
