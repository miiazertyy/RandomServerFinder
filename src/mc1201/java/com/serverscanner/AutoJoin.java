...
public final class AutoJoin {
	 ...
	 */
	public static void stopAndReturn(MinecraftClient client, Screen from) {
		cancelConnecting(from);
		stop();
		client.setScreen(new ServerScannerScreen(new MultiplayerScreen(new TitleScreen())));
	}
	...
	private static void cancelConnecting(Screen from) {
		...
		for (Field field : ConnectScreen.class.getDeclaredFields()) {
			...
			try {
				...
				} else if (ClientConnection.class.isAssignableFrom(field.getType())) {
					field.setAccessible(true);
					ClientConnection connection = (ClientConnection) field.get(from);
					// ConnectScreen.ABORTED_TEXT is package-private here; this is the same string.
					if (connection != null) connection.disconnect(Text.translatable("connect.aborted"));
				}
			} catch (Exception e) {
	...
	private static void tick(MinecraftClient client) {
		...
		if (client.currentScreen instanceof ConnectScreen) return;
	...
	private static void connect(MinecraftClient client, ScannedServer server) {
		...
		ConnectScreen.connect(parent, client, ServerAddress.parse(address), info, false);
	}
