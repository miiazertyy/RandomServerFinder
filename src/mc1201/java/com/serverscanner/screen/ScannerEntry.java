...
import com.serverscanner.local.PingState;
...
public class ScannerEntry {
	...
	/** 1.20.1's server info cannot hold this, so the row owns it. See {@link PingState}. */
	private PingState state = PingState.INITIAL;
	...
	public ScannerEntry(ScannedServer server) {
		...
		if (server.resolved != null) {
			...
			// Written alongside resolved by the feed. Absent only for a row restored from history,
			// which was reachable by definition or it would never have been joined.
			this.state = server.pingState instanceof PingState carried ? carried : PingState.SUCCESSFUL;
			return;
		}

		this.serverInfo = new ServerInfo(server.address(), server.address(), false);
	...
	public boolean isUnreachable() {
		return pingFinished && state == PingState.UNREACHABLE;
	}

	public boolean isReachable() {
		return pingFinished && (state == PingState.SUCCESSFUL || state == PingState.INCOMPATIBLE);
	}

	/** How this row's ping ended, for the latency icon. */
	...
	void onPingSucceeded() {
		...
		state = com.serverscanner.local.ViaFabricPlusBridge.isAvailable()
