...
public class ServerScannerScreen extends Screen {
	...
	private void openDirectConnect() {
		...
		ServerInfo target = new ServerInfo(name, address, false);
		...
		this.client.setScreen(new DirectConnectScreen(this, confirmed -> {
			...
			releaseIcons();
			ConnectScreen.connect(this, this.client, ServerAddress.parse(existing.address), existing, false);
		}, target));
	}
	...
	private void saveToServerList() {
		...
		servers.add(new ServerInfo(selected.displayName(), address, false), false);
		servers.saveFile();
		savedToServerList = true;
	}
