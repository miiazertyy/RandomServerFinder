...
public class ScannerListWidget {
	...
	private void renderCard(DrawContext context, ScannerEntry entry, int index, int cardX, int cardY,
		...
		context.drawTexture(entry.getIconTexture(), iconX, iconY, 0, 0.0F, 0.0F, ICON_SIZE, ICON_SIZE, ICON_SIZE, ICON_SIZE);
		...
		if (status != null) {
			context.drawTexture(status, cardX + w - 15, cardY + 6, 0, 0, 10, 8);
		}
	}
	...
	private void renderEntry(DrawContext context, ScannerEntry entry, int index, int entryY, int mouseX, int mouseY) {
		...
		context.drawTexture(entry.getIconTexture(), iconX, entryY, 0, 0.0F, 0.0F, ICON_SIZE, ICON_SIZE, ICON_SIZE, ICON_SIZE);
		...
		Identifier status = statusTexture(entry, index);
		if (status != null) {
			context.drawTexture(status, statusX, entryY + 1, 0, 0, 10, 8);
		}
	...
	private void renderHoverTooltips(DrawContext context, ScannerEntry entry, int mouseX, int mouseY,
		...
		if (mouseX >= statusX && mouseX <= statusX + 10 && mouseY >= entryY + 1 && mouseY <= entryY + 9) {
			ServerInfo info = entry.getServerInfo();
			Text tooltip = switch (entry.getState()) {
		...
		if (summary != null && !summary.isEmpty()
			...
			context.drawTooltip(client.textRenderer, summary, mouseX, mouseY);
		}
	}
	...
	public boolean mouseScrolled(double mouseX, double mouseY, double amount) {
		...
		targetScroll = clampScroll(targetScroll - amount * rowHeight() * SCROLL_ROWS_PER_NOTCH);
		return true;
	}
	...
	public boolean mouseDragged(double mouseX, double mouseY, int button, double deltaX, double deltaY) {
		...
		int handleHeight = getScrollbarHandleHeight(maxScroll);
		double ratio = (mouseY - y - dragOffset) / (height - handleHeight);
		targetScroll = clampScroll(ratio * maxScroll);
		scroll = targetScroll;
		return true;
	}
	...
	static void drawStatusIcon(DrawContext context, Identifier texture, int x, int y) {
		context.drawTexture(texture, x, y, 0, 0, 10, 8);
	}
