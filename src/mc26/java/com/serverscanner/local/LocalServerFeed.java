...
public class LocalServerFeed {
	...
	private static final class Candidate {
		...
		Candidate(ServerData info, long ip, int port, boolean bedrock, boolean neighbour) {
			this.info = info;
			this.ip = ip;
			this.port = port;
	...
	private void startPing(long ip, int port, boolean bedrock, boolean neighbour) {
		...
		info.setState(ServerData.State.PINGING);
		...
		executor.submit(() -> {
			try {
				...
				client.execute(() -> info.setState(ServerData.State.UNREACHABLE));
			}
		});
	}
	...
	private void harvest() {
		...
		while (it.hasNext()) {
			Candidate c = it.next();
			ServerData.State status = c.info.state();
			boolean timedOut = now - c.startedAt > PING_TIMEOUT_MS;

			if (status == ServerData.State.PINGING && !c.answered && !timedOut) {
				continue;
			}
			it.remove();

			// A server that sent its details but no latency reply is still perfectly joinable.
			boolean responded = c.answered || c.info.getIconBytes() != null || c.info.players != null;
			if (!responded || status == ServerData.State.UNREACHABLE) {
				continue;
			}
			...
			c.info.setState(ViaFabricPlusBridge.isAvailable()
					|| c.info.protocol == net.minecraft.SharedConstants.RELEASE_NETWORK_PROTOCOL_VERSION
	...
	private ScannedServer toScannedServer(Candidate c) {
		...
		server.version.protocol = c.info.protocol;

		server.description = c.info.motd == null ? "" : c.info.motd.getString();
		server.rawDescription = server.description;

		ServerStatus.Players players = c.info.players;
		...
		server.hasFavicon = c.info.getIconBytes() != null;
	 ...
	 */
	private void startBedrockPing(Candidate candidate, ServerData info, long ip, int port) {
		...
		executor.submit(() -> {
			...
			try {
				...
				client.execute(() -> {
					info.protocol = pong.protocol();
					info.version = net.minecraft.network.chat.Component.literal(pong.version().isBlank() ? "Bedrock" : "Bedrock " + pong.version());
					info.motd = net.minecraft.network.chat.Component.literal(pong.name());
					...
					info.players = new net.minecraft.network.protocol.status.ServerStatus.Players(pong.max(), pong.online(), java.util.List.of());
					info.status = net.minecraft.network.chat.Component.literal(pong.online() + "/" + pong.max());
					info.ping = latency;
