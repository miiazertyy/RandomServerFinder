 ...
 * the screen's render event instead of by subclassing one — the drawing hooks on {@code Button}
...
public final class ScannerScreenHooks {
	...
	public static void register() {
		...
		ScreenEvents.AFTER_INIT.register((client, screen, scaledWidth, scaledHeight) -> {
			...
			Button existing = findByLabel(buttons, FINDER_LABEL);
			final Button button;
			if (existing != null && buttons.contains(existing)) {
				...
				button = Button
						.builder(Component.literal(FINDER_LABEL),
								b -> client.setScreenAndShow(new ServerScannerScreen(screen)))
						.bounds(findFreeX(buttons, scaledHeight, null), y, BUTTON_SIZE, BUTTON_SIZE)
						.tooltip(Tooltip.create(Component.translatable("randomserverfinder.tip.random_server_finder_nfind_a")))
						.build();
				buttons.add(button);
			}
			...
			ScreenEvents.afterExtract(screen).register((rendered, context, mouseX, mouseY, delta) -> {
	...
	private static Button findByLabel(List<AbstractWidget> widgets, String label) {
		for (AbstractWidget widget : widgets) {
			if (widget instanceof Button button && label.equals(button.getMessage().getString())) {
				return button;
			}
		}
		return null;
	...
	private static void registerStopButton() {
		ScreenEvents.AFTER_INIT.register((client, screen, scaledWidth, scaledHeight) -> {
			...
			if (findByLabel(Screens.getWidgets(screen), STOP_LABEL) == null) {
				Screens.getWidgets(screen).add(Button
						.builder(Component.literal(STOP_LABEL),
								b -> AutoJoin.stopAndReturn(client, screen))
						.bounds(6, 6, 110, 20)
						.tooltip(Tooltip.create(Component.translatable("randomserverfinder.tip.stop_trying_servers_drop_the")))
						.build());
			}
			...
			if (!(screen instanceof AutoJoinQueueScreen)
					&& findByLabel(Screens.getWidgets(screen), QUEUE_LABEL) == null) {
				Screens.getWidgets(screen).add(Button
						.builder(Component.literal(QUEUE_LABEL),
								b -> client.setScreenAndShow(new AutoJoinQueueScreen(screen)))
						.bounds(6, MARGIN + 24, 110, 20)
						.tooltip(Tooltip.create(Component.translatable("randomserverfinder.tip.see_what_is_lined_up")))
						.build());
			}
	 ...
	 */
	private static void drawStatus(GuiGraphicsExtractor context, Minecraft client, int x, int y) {
		...
		int textWidth = client.font.width(headline);
		if (!tail.isEmpty()) textWidth += 10 + client.font.width(tail);
		...
		context.text(client.font, headline, x + 9, y + 6, STATUS_TEXT);
		if (!tail.isEmpty()) {
			context.text(client.font, tail,
					x + width - 8 - client.font.width(tail), y + 6, STATUS_DIM);
		}
	 ...
	 */
	private static int findFreeX(List<AbstractWidget> existing, int scaledHeight, AbstractWidget self) {
		int y = scaledHeight - BUTTON_SIZE - MARGIN;
		int x = MARGIN;
	 ...
	 */
	private static void drawDice(GuiGraphicsExtractor context, int buttonX, int buttonY, boolean hovered) {
	 ...
	 */
	private static void drawCube(GuiGraphicsExtractor context, int ox, int oy, int top, int left, int right) {
		int half = CUBE_W / 2;
	 ...
	 */
	private static void drawPips(GuiGraphicsExtractor context, int ox, int oy, int face, int side) {
