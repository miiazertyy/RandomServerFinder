...
public class ScannerListWidget {
	...
	private static final Identifier[] PING_TEXTURES = {
			Identifier.withDefaultNamespace("server_list/ping_1"),
			Identifier.withDefaultNamespace("server_list/ping_2"),
			Identifier.withDefaultNamespace("server_list/ping_3"),
			Identifier.withDefaultNamespace("server_list/ping_4"),
			Identifier.withDefaultNamespace("server_list/ping_5"),
	};
	private static final Identifier[] PINGING_TEXTURES = {
			Identifier.withDefaultNamespace("server_list/pinging_1"),
			Identifier.withDefaultNamespace("server_list/pinging_2"),
			Identifier.withDefaultNamespace("server_list/pinging_3"),
			Identifier.withDefaultNamespace("server_list/pinging_4"),
			Identifier.withDefaultNamespace("server_list/pinging_5"),
	};
	 ...
	 */
	private void renderCard(GuiGraphicsExtractor context, ScannerEntry entry, int index, int cardX, int cardY,
		...
		context.blit(RenderPipelines.GUI_TEXTURED, entry.getIconTexture(), iconX, iconY,
		...
		if (JoinedServers.get().hasJoined(server.address())) {
			context.text(client.font, Component.translatable("randomserverfinder.x"), nameX, cardY + 6, 0xFF5BD16F);
			nameX += client.font.width("✔ ");
		}
		...
		List<FormattedCharSequence> lines = client.font.split(motd, textWidth);
		if (!lines.isEmpty()) {
			context.text(client.font, lines.get(0), textX, cardY + 18,
					dead ? COLOR_DIM : COLOR_MOTD);
		}

		Component countText = info.status != null ? info.status
				: Component.literal(server.onlinePlayers() + "/" + server.maxPlayers());
		context.text(client.font, countText, textX, cardY + 30, COLOR_META);

		context.text(client.font,
				client.font.plainSubstrByWidth(server.versionName(), textWidth - 24),
				textX, cardY + 41, COLOR_META);

		Identifier status = statusTexture(entry, index);
		if (status != null) {
			context.blitSprite(RenderPipelines.GUI_TEXTURED, status, cardX + w - 15, cardY + 6, 10, 8);
		}
	}

	private void renderEntry(GuiGraphicsExtractor context, ScannerEntry entry, int index, int entryY, int mouseX, int mouseY) {
		ScannedServer server = entry.getServer();
		ServerData info = entry.getServerInfo();
		...
		context.blit(RenderPipelines.GUI_TEXTURED, entry.getIconTexture(), iconX, entryY, 0.0F, 0.0F,
		...
		int labelX = textX;
		if (joined) {
			context.text(client.font, Component.translatable("randomserverfinder.x"), labelX, entryY + 1, 0xFF5BD16F);
			labelX += client.font.width("✔ ");
		}
		...
		Component countText = entry.isReachable() && info.status != null
				? info.status
				: Component.literal(server.onlinePlayers() + "/" + server.maxPlayers());
		...
		Identifier status = statusTexture(entry, index);
		if (status != null) {
			context.blitSprite(RenderPipelines.GUI_TEXTURED, status, statusX, entryY + 1, 10, 8);
		}
		context.text(client.font, countText, countX, entryY + 1, dead ? COLOR_DIM : COLOR_MOTD);
		...
		List<FormattedCharSequence> lines = client.font.split(motd, motdWidth);
		for (int i = 0; i < Math.min(lines.size(), 2); i++) {
			context.text(client.font, lines.get(i), textX, entryY + 12 + 9 * i,
					dead ? COLOR_DIM : COLOR_MOTD);
		}

		// Metadata line only fits when the MOTD used a single line.
		if (lines.size() <= 1) {
			context.text(client.font, metaLine(server), textX, entryY + 21 + 5, COLOR_META);
		}
	...
	private void renderHoverTooltips(GuiGraphicsExtractor context, ScannerEntry entry, int mouseX, int mouseY,
			int statusX, int entryY, int countX, int countWidth) {
		if (mouseX >= statusX && mouseX <= statusX + 10 && mouseY >= entryY + 1 && mouseY <= entryY + 9) {
			ServerData info = entry.getServerInfo();
			Component tooltip = switch (info.state()) {
				case SUCCESSFUL -> Component.translatable("multiplayer.status.ping", info.ping);
				...
				default -> Component.translatable("multiplayer.status.pinging");
			};
			context.setTooltipForNextFrame(tooltip, mouseX, mouseY);
			return;
		}

		List<Component> summary = entry.getPlayerListSummary();
		if (summary != null && !summary.isEmpty()
				&& mouseX >= countX && mouseX <= countX + countWidth
				&& mouseY >= entryY + 1 && mouseY <= entryY + 10) {
			List<FormattedCharSequence> ordered = new ArrayList<>(summary.size());
			for (Component t : summary) ordered.add(t.getVisualOrderText());
			context.setTooltipForNextFrame(ordered, mouseX, mouseY);
		}
	}
	...
	private Identifier statusTexture(ScannerEntry entry, int index) {
		...
		return switch (info.state()) {
			...
			case PINGING -> {
				int frame = (int) (System.currentTimeMillis() / 100L + index * 2L & 7L);
	 ...
	 */
	private void drawScrolling(GuiGraphicsExtractor context, String text, int x, int y, int available, int colour) {
		...
		context.text(client.font, text, x - offset, y, colour);
		context.text(client.font, text, x - offset + cycle, y, colour);
		context.disableScissor();
	}

	private void renderScrollbar(GuiGraphicsExtractor context, int maxScroll) {
