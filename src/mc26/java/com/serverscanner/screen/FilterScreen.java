...
public class FilterScreen extends Screen {
	...
	private static final class Item {
		...
		Item(String text, AbstractWidget widget, Supplier<Boolean> isSet) {
			this.text = text;
			this.widget = widget;
			this.isSet = isSet;
		}
	...
	public FilterScreen(Screen parent, Page page) {
		super(Component.literal(page == Page.FILTERS ? "Server Filters" : "Settings"));
		this.parent = parent;
		this.page = page;
	}
	...
	protected void init() {
		...
		for (Item item : items) {
			if (item.widget != null) {
				addRenderableWidget(item.widget);
			}
		}

		int centre = this.width / 2;
		int buttonY = this.height - 27;
		addRenderableWidget(Button.builder(Component.translatable("randomserverfinder.reset_all"), b -> {
					filters.reset();
					this.rebuildWidgets();
				})
				.bounds(centre - 154, buttonY, 100, 20).build());

		addRenderableWidget(Button.builder(Component.translatable("randomserverfinder.cancel"), b -> cancel())
				.bounds(centre - 50, buttonY, 100, 20).build());

		addRenderableWidget(Button.builder(Component.translatable("randomserverfinder.done"), b -> this.onClose())
				.bounds(centre + 54, buttonY, 100, 20).build());
	...
	private void section(String titleKey) {
		String title = net.minecraft.client.resources.language.I18n.get(titleKey);
	...
	private void text(int x, String[] icon, String labelKey, String tooltipKey, Supplier<String> get, Consumer<String> set) {
		...
		EditBox field = new EditBox(this.font, x, 0, FIELD_WIDTH, FIELD_HEIGHT,
				Component.literal(label));
		field.setMaxLength(256);
		field.setValue(get.get());
		field.setResponder(set);
	...
	private void tri(int x, String[] icon, String labelKey, String tooltipKey, Supplier<TriState> get, Consumer<TriState> set) {
		...
		holder[0] = Button.builder(triText(get.get()), b -> {
					TriState next = get.get().next();
					set.accept(next);
					holder[0].setMessage(triText(next));
				})
				.bounds(x, 0, FIELD_WIDTH, FIELD_HEIGHT)
	...
	private void toggle(int x, String[] icon, String labelKey, String tooltipKey, Supplier<Boolean> get, Consumer<Boolean> set) {
		...
		String tooltip = net.minecraft.client.resources.language.I18n.get(tooltipKey);
		Button[] holder = new Button[1];
		holder[0] = Button.builder(onOffText(get.get()), b -> {
	...
	private void cycle(int x, String[] icon, String labelKey, String tooltipKey, Supplier<String> display, Runnable advance) {
		...
		String tooltip = net.minecraft.client.resources.language.I18n.get(tooltipKey);
		Button[] holder = new Button[1];
		holder[0] = Button.builder(Component.literal(display.get()), b -> {
					advance.run();
					holder[0].setMessage(Component.literal(display.get()));
				})
				.bounds(x, 0, FIELD_WIDTH, FIELD_HEIGHT)
				.tooltip(Tooltip.create(Component.literal(tooltip)))
				.build();
	...
	private static Component triText(TriState state) {
		return switch (state) {
			case UNSET -> Component.translatable("randomserverfinder.any").withStyle(ChatFormatting.DARK_GRAY);
			case TRUE -> Component.translatable("randomserverfinder.yes").withStyle(ChatFormatting.GREEN);
			case FALSE -> Component.translatable("randomserverfinder.no").withStyle(ChatFormatting.RED);
		};
	}

	private static Component onOffText(boolean value) {
		return value ? Component.translatable("randomserverfinder.on").withStyle(ChatFormatting.GREEN)
				: Component.translatable("randomserverfinder.off").withStyle(ChatFormatting.DARK_GRAY);
	}
	...
	public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float deltaTicks) {
		...
		// Header.
		com.serverscanner.compat.Draw.centered(context, this.font, this.title, this.width / 2, 12, 0xFFFFFFFF);
	...
	private void renderItems(GuiGraphicsExtractor context, int mouseX, int mouseY, int px, int top, int bottom) {
		...
		for (Item item : items) {
			...
			if (item.isSection()) {
				int textY = item.y + 12;
				context.text(this.font, Component.literal(item.text.toUpperCase()),
						px + 12, textY, SECTION_TEXT);
				int ruleX = px + 16 + this.font.width(item.text.toUpperCase());
			...
			context.text(this.font, Component.literal(item.text),
					labelX, item.y + (FIELD_ROW - 8) / 2, set ? LABEL_TEXT_SET : LABEL_TEXT);
		}
	}

	private void renderScrollbar(GuiGraphicsExtractor context, int px, int top, int bottom) {
	...
	public boolean mouseClicked(MouseButtonEvent click, boolean doubled) {
		...
		if (click.y() < viewTop() || click.y() >= viewBottom()) {
			for (Item item : items) {
				AbstractWidget w = item.widget;
				if (w != null && w.visible && click.y() >= w.getY() && click.y() < w.getY() + FIELD_HEIGHT) {
					return false;
				}
			}
		}
